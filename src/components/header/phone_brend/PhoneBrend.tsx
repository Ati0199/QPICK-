import "./PhoneBrend.css";
import { useMemo, useRef, useState } from "react";

import PhoneBrendList from "./phone_brend_list/PhoneBrendList";
import Service from "../../../service/service";
import DropDownSkeleton from "../../drop-down-skeleton/DropDownSkeleton";
import { type IPhoneData } from "../../../types";

export default function PhoneBrend() {
  const [click, setClick] = useState(false);
  const [phoneData, setPhoneData] = useState<IPhoneData[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [openedBrendId, setOpenedBrendId] = useState(null);
  ...
  return (
    <>
      <div className="choose_phone_block">
        ...
        <label
          className="choose_phone_brend"
          onClick={() => {
            const newClick = !click;
            setClick(newClick);
            if (newClick) {
              setIsLoading(true);
              s.getPhoneData().then((data) => {
                setPhoneData(data);
                setIsLoading(false);
              });
            }
        ...
        >
          ...
          <img
            className={` ${click ? "animation_up" : "animation_down"}`}
            src="/drop_down.svg"
            alt=""
          />
        </label>
        ...
        {click ? (
          <div ref={dropDownRef} className="phone_models">
            {isLoading ? (
              <DropDownSkeleton />
            ) : (
              phoneData.map((item) => {
                return (
                  <PhoneBrendList
                    ...
                    id={item.id}
                    isLoading={isLoading}
                    openedBrendId={openedBrendId}
                    setOpenBrendId={setOpenedBrendId}
                  />
                );
              })
            )}
